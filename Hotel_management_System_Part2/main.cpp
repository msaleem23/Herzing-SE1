...
void Room::deleteRoom(int rno)
{
    for (int i = 0; i < Room::count; i++) {
        if (rooms[i].room_number == rno) {
            // Mark room as deleted or remove from array (you may decide how to handle this)
            rooms[i].status = -1; // Mark it as deleted
            cout << "Room " << rno << " deleted successfully." << endl;
            return;
        }
    }
...
void Room::searchRoom(int rno)
{
    for (int i = 0; i < Room::count; i++) {
        if (rooms[i].room_number == rno) {
            ...
            cout << "Room Rent: " << rooms[i].rent << endl;
            cout << "Room Capacity: " << rooms[i].capacity << endl;
            cout << "Room Status: " << (rooms[i].status == 0 ? "Available" : "Occupied") << endl;
            return;
        }
    }
    cout << "Room not found." << endl;
